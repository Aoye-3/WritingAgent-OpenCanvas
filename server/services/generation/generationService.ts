    ...
    contextValues: {
      ...
      facetwrite_clarification_policy: {
        ...
        instruction: payload.locale === "zh"
          ? `当前文献/数据库类任务还处于搜索前 intake 阶段。你必须依据已加载 Skill 调用 ask_clarification，最多进行 ${MAX_SKILL_INTAKE_ROUNDS} 轮高价值澄清；当前是第 ${intake.nextRound} 轮。已回答摘要：${answeredSummary}。已回答槽位：${answeredSlots}。仍缺失：${missingSlots}。只能围绕仍缺失槽位提问，不要重复询问已回答槽位；若缺失槽位为 none，请继续执行任务而不是继续澄清。追问质量策略：采用苏格拉底式追问，诊断会改变检索范围、证据质量、综述结构或交付验收的关键假设；优先询问仍缺失槽位中影响最大的一个，而不是低价值泛确认。每轮只问一个最有价值的问题，参数必须包含 question 和 2-3 个 options；每个 option 必须有 id、label、detail 或 description，且互斥、可执行、能直接改变下一步检索或综述策略，最多一个 recommended:true。不要输出普通澄清话术、Markdown 选项列表或自然语言阶段说明；如果无法调用工具，只能输出完整 JSON 对象 {"type":"agent_clarification_requested","question":"...","options":[...]}，不能包含任何额外文本。在用户回答前不要调用 web_search、web_fetch、knowledge_base、write_file、present_files 或其他证据工具。`
          : `This literature/database task is still in the pre-search intake stage. Use the loaded Skill instructions to call ask_clarification, for up to ${MAX_SKILL_INTAKE_ROUNDS} high-value clarification rounds; this is round ${intake.nextRound}. Answers so far: ${answeredSummary}. Answered slots: ${answeredSlots}. Missing slots: ${missingSlots}. Ask only about the missing slots and do not repeat answered slots; if missing slots is none, continue the task instead of clarifying again. Question quality strategy: use Socratic clarification to diagnose the assumption that most changes search scope, evidence quality, synthesis structure, or delivery acceptance. Prioritize the highest-impact missing slot instead of low-value generic confirmation. Ask only one highest-value question per round, and the arguments must include question plus 2-3 options; every option must include id, label, and detail or description, be mutually exclusive, actionable, and directly useful for the next search or synthesis step, with at most one recommended:true. Do not output ordinary clarification prose, Markdown option lists, or phase narration. If tool calling is unavailable, output only a complete JSON object {"type":"agent_clarification_requested","question":"...","options":[...]} with no surrounding text. Do not call web_search, web_fetch, knowledge_base, write_file, present_files, or other evidence tools until intake is sufficient.`
      }
    }
  };
  return withServerDurableContinuationContext(
