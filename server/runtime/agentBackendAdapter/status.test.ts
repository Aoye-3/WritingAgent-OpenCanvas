...
test("reports TypeScript fallback when AgentBackend is disabled", async () => {
  const status = await getAgentBackendRuntimeStatus({
...
test("reports AgentBackend unreachable without throwing", async () => {
  const status = await getAgentBackendRuntimeStatus({
    config: { enabled: true, baseUrl: "http://AgentBackend.local", assistantId: "lead_agent" },
