...
test("reads AgentBackend skills and sanitizes MCP secrets", async () => {
  ...
  const overview = await getAgentBackendConfigOverview({
    config: authConfig(),
    fetchImpl
  });
...
test("returns safe error shape when AgentBackend config is unreachable", async () => {
  clearAgentBackendSession();
  const overview = await getAgentBackendConfigOverview({
    config: authConfig(),
