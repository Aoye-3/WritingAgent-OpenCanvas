...
export function registerInternalDeerFlowRoutes(app: Express, { storage }: InternalDeerFlowRouteDeps) {
  app.post("/api/internal/deerflow/tool-call", async (request, response) => {
    ...
    try {
      ...
      sendError(response, 400, "bad_request", errorMessage(error, "Unable to execute DeerFlow tool bridge call"));
    }
  });
}
...
function isAllowedInternalRequest(request: Request) {
  ...
  return request.header("x-facetwrite-internal") === "deerflow";
}
