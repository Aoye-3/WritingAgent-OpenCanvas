...
export function registerInternalAgentBackendRoutes(app: Express, { storage, knowledgeService }: InternalAgentBackendRouteDeps) {
  app.post("/api/internal/agent-backend/tool-call", async (request, response) => {
    if (!isAllowedInternalRequest(request)) {
      sendError(response, 403, "validation_failed", "AgentBackend tool bridge calls require an internal source.");
      return;
    }
    ...
    try {
      ...
      sendError(response, 400, "bad_request", errorMessage(error, "Unable to execute AgentBackend tool bridge call"));
    }
  });
}
...
function isAllowedInternalRequest(request: Request) {
  ...
  return request.header("x-facetwrite-internal") === "agent-backend";
}
...
function toToolCall(input: BridgeRequest): ChatToolCall {
  return {
    id: `agent_backend_${input.toolName}`,
    type: "function",
    function: {
      name: input.toolName,
