...
import { registerInternalDeerFlowRoutes } from "./internalDeerFlowRoutes.js";
...
async function request(app: express.Express, body: unknown, headers: Record<string, string> = {}) {
  ...
  try {
    ...
    assert.ok(address);
    const response = await fetch(`http://127.0.0.1:${(address as AddressInfo).port}/api/internal/deerflow/tool-call`, {
      method: "POST",
...
test("rejects DeerFlow tool bridge calls without an allowed internal source", async () => {
...
test("canvas_write bridge calls create pending requests only", async () => {
  ...
  const result = await request(app, {
    ...
    arguments: {
      ...
      nodeKind: "document",
      title: "Draft",
      content: "Pending via DeerFlow",
      rationale: "Requested by DeerFlow"
    },
