...
import { getDeerFlowDashboard } from "./dashboard.js";
...
test("dashboard returns runtime, config, agent mapping, and tool bridge status", async () => {
  ...
  const dashboard = await getDeerFlowDashboard({
...
test("dashboard shows fallback state when DeerFlow is disabled", async () => {
  const dashboard = await getDeerFlowDashboard({
    agentRuntime: fakeAgentRuntime(),
