...
import { authenticatedDeerFlowFetch, clearDeerFlowSession, getDeerFlowAuthStatus } from "./auth.js";
import type { DeerFlowRuntimeConfig } from "./config.js";

const config: DeerFlowRuntimeConfig = {
  enabled: true,
...
test("reports setup required when auto setup is disabled", async () => {
  ...
  const status = await getDeerFlowAuthStatus({
    config,
    fetchImpl: async (url) => {
      assert.equal(String(url), "http://deerflow.local/api/v1/auth/setup-status");
...
test("auto setup initializes admin and caches session", async () => {
  ...
  assert.deepEqual(calls, [
    ...
    "POST http://deerflow.local/api/v1/auth/initialize"
  ]);
});
