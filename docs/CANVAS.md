...
This scope exists so the Agent can still write useful short Canvas material during a long task without using ordinary nodes as a long-document transport. The execution layer enforces the contract. Oversized content, long-form titles, destructive operations, and `file_document` node kinds fail and instruct the Agent to use `write_file` and `present_files`. Scoped low-risk creates use stable ids derived from the Thread and title so repeated same-title summary/reference writes update one node rather than creating duplicate cards. Explicit user Canvas actions with `canvasAction.requiresTool` still expose `canvas_write`, but they do not clear the progressive short-node scope during long-task delivery. Tool-call arguments remain the source of truth for operation selection: an explicit `operation:"create"` or `operation:"append"` must not be upgraded by stale outer `canvasAction.operation` context. `canvasAction.operation` is only a fallback when the tool call omitted `operation`. If a fallback or explicit `replace` or `delete` has no resolved target from `canvasAction.targetNodeId`, tool args, or the selected Canvas node, Tool Runtime returns a recoverable `canvas_mutation_failed` diagnostic with `reason:"missing_target_node"` and does not call the storage write-request path. Targeted destructive operations outside this guard still require approval. Skill scope guard runs are stricter and expose only `ask_clarification`.
...
If a run uses `write_file` or `present_files` for `/mnt/user-data/outputs/*.md`, progressive delivery must create or update the stable `file_document` node for that virtual path. `present_files` marks the node as ready to preview. Runtime-reported Markdown paths are authoritative: if artifact archiving reports a failure but the current Thread outputs directory already contains a readable Markdown file for that virtual path, finalization still treats the real output as previewable. Backend fallback Markdown is created only when no real Markdown path was reported, or every reported Markdown path is unreadable. A fallback `facetwrite-delivery-*.md` file must never replace or shadow a readable Runtime-authored Markdown output. If a medium/long progressive run finishes without Runtime file tools, backend finalization writes the final Markdown to the current thread outputs directory and creates the same compact node. Multiple writes to the same Markdown file update one node instead of creating duplicates.
...
The Markdown preview panel builds its document switcher from the current Project Canvas `file_document` nodes. It does not scan the outputs directory for orphan files. Switching documents reuses the same preview API with the selected node's `metadata.fileDocument.threadId` and `metadata.fileDocument.path`, clears the previous source selection, and reloads Claim candidates for that exact document path.
...
Canvas action recognition must only classify replace/delete when the user clearly targets Canvas state, a node, current content, or a selected card. Examples such as "覆盖当前节点", "替换画布内容", "重写选中的卡片", and "overwrite canvas/node" are replace intents. Content-scope phrases such as "文献全覆盖", "覆盖 2026 年论文", or "覆盖经典与前沿" describe research breadth and must not become Canvas replace/delete. When in doubt, prefer no Canvas action or low-risk create over destructive intent.

The server-recognized `canvasAction` is safety context, not an override for explicit tool parameters. Tool Runtime resolves `canvas_write.operation` in this order: explicit tool `args.operation`, then `context.canvasAction.operation` fallback, then the existing low-risk default behavior. A stale fallback replace/delete without a target must fail safely with `missing_target_node`; an explicit low-risk create/append should still commit through the direct low-risk path.
