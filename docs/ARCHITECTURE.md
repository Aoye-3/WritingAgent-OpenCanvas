...
The shell uses a single-instance lock, checks ports before startup, resolves `local`, `docker`, or `external` Runtime mode, waits for each health endpoint, and opens the main window only after readiness. The double-click `start-opencanvas-shell.vbs` entry explicitly forces `local` mode and lets the managed launcher choose an available Gateway port unless `AGENT_RUNTIME_PORT` is set. Docker mode remains an explicit command/configuration path. Local reuse requires compatible project, port, source fingerprint, and ToolUse bridge metadata. Partial or incompatible runtimes block startup. Shutdown attempts every owned cleanup step and never stops a reused or external runtime.
...
## Harness Source Update Boundary

Harness/App Shell updates are application updates, not user-data synchronization. The first supported update model is a source Git update for the current development checkout, using the allowlisted `origin/main` channel. It must not clone, worktree, mirror, copy repositories, or synchronize arbitrary GitHub URLs into the workspace.

Electron/App Shell owns `git fetch --prune origin`, preview generation, fast-forward apply, dependency installation, service shutdown, and restart orchestration. Apply requires a clean worktree, non-detached HEAD, allowlisted origin remote, expected HEAD match, protected-path check, and a fast-forward target. It must not stash, rebase, reset, create merge commits, or resolve conflicts automatically. The Express backend may expose update status or preview data later, but it must not run Git or overwrite its own running code in-process. The renderer only presents check, preview, apply, progress, failure, and restart controls through Shell IPC.

The update surface may include Git-tracked application shell code, frontend/backend source, Agent Runtime source, built-in Skills, demo/example/template assets, docs, and default config examples. It must never write `.facetwrite/**`, a `FACETWRITE_APP_ROOT` data directory, `.env*`, `modules/agent-runtime/.env`, provider API stores, SQLite database files, Thread user-data, Knowledge vectors/uploads, Memory, Project thumbnails, user uploads/outputs, dependency folders, or runtime/test temp roots. Database changes after an application update must run through versioned migrations; source updates and future release artifacts must not replace user database files.

Future packaged builds may add a GitHub Release artifact updater, but that is a separate distribution-stage path sharing the same protected-data boundary.
...
- `src/app/hooks/useGenerationRun.ts` owns structured generation, chat generation, streaming token/status/tool-event updates, versions, collaboration messages, and direct Canvas-write intent handoff. For chat streaming it creates a temporary assistant message in the right AI collaboration drawer, fills that assistant bubble through a typewriter queue, aggregates repeated same-tool events into that message status, then reconciles with persisted thread state after the final response. Canvas and Artifact lifecycle tool events trigger a live Thread-state refresh for Canvas/Plan surfaces while text is still streaming. Progress Canvas delivery commits (`research` cards and `Body draft` checkpoints) apply their local node snapshot immediately and debounce full Thread-state refreshes; final, file, sources, failed, artifact, approval, and clarification events bypass that debounce. The main Canvas layout is not a streaming transcript surface. When the user explicitly asks to write to Canvas, it auto-approves only the new pending write requests created by that run.
...
- Home project cards prefer local cached Project thumbnail images when available. The desktop shell captures the visible Canvas area when a user leaves or switches the active Project, posts the image to `/api/projects/:projectId/thumbnail`, and Home loads the conventional thumbnail URL with a cache-busting token. If the image is missing, fails, or the browser is not running inside the shell, Home falls back to `ProjectSummary.canvasPreview`. The fallback renderer maps node/object geometry into the card area and then falls back to the static placeholder when the Project has no preview data. Home must not fetch full Canvas state per card.
- `src/features/projects/ProjectsView.tsx` owns Project management sorting. The Project table keeps the backend Project list intact but applies local sorting after search/Agent filters. `Assets` and `Updated` headers are single-badge toggle buttons; clicking a header switches between ascending and descending order so users can quickly group empty or stale Projects for selection and trash/delete workflows.
...
- React Flow controlled-node updates must preserve `dimensions` changes and existing `measured.width/height` values. Dropping those view-only measurements makes React Flow treat visible nodes as uninitialized during drag, causing interaction jank even when persisted Canvas geometry is correct.
...
- `src/features/settings/SourceUpdateView.tsx` renders the left-navigation Harness update page. It shows Shell-only source Git preview/apply state and stays separate from the Project Settings dialog.
...
- `server/toolRuntime.ts` executes local ToolUse behavior and creates Canvas write requests when `canvas_write` is called. If a model asks for `replace` without an explicit user replace/overwrite instruction, the runtime normalizes the operation to `append` for a selected node or `create` otherwise. If a server-forced replace/delete has no target node, Tool Runtime returns `reason:"missing_target_node"` before the storage write-request layer.
...
- FacetWrite calls the current AgentBackend adapter as an independent Python Gateway over HTTP/SSE when `AGENT_BACKEND_ENABLED=true`. The Gateway exposes LangGraph-compatible runs endpoints, with `lead_agent` registered in `modules/agent-runtime/backend/langgraph.json` and implemented by `deerflow.agents:make_lead_agent`.
- The recommended local path is the project-managed Python Gateway on a launcher-managed `127.0.0.1` port; direct low-level debugging can still pin `AGENT_RUNTIME_PORT`. Docker Compose through nginx at `http://127.0.0.1:2026` is an explicit isolation and deployment mode.
- Progressive long-task Canvas delivery is server-owned but does not remove `canvas_write`. Instead, the runtime receives `facetwrite_canvas_write_scope:"short_progress_nodes"` so `canvas_write` can create short summaries, overviews, progress/reference notes, and references while rejecting long bodies, full reports, untargeted destructive operations, and `file_document` creation. `canvasAction.requiresTool` does not clear this scope during progressive delivery; targeted destructive writes still require approval. Full Markdown deliverables must use `write_file` plus `present_files`; the server then creates or updates the compact `file_document` preview node.
- Agent Runtime clarification is a native checkpoint pause. The Python middleware calls LangGraph `interrupt()` with a sanitized structured payload, the Gateway resumes with `Command(resume=...)`, and FacetWrite keeps the same runtime `thread_id` instead of starting a new ordinary chat run. `agent_backend_agent_clarification_requested` remains the persisted product projection for the UI, but raw checkpoint state, prompts, reasoning, and tool arguments are not exposed to messages or output versions.
- Durable task premature exit is not a LangGraph interrupt. `DurableTaskGuardMiddleware` may continue a pure process promise inside the same graph at most twice; exhaustion emits `durable_task_incomplete`. FacetWrite applies a stricter readiness verdict before direct/progressive Canvas finalization, `run_completed`, or Plan completion, so process text and placeholders cannot become terminal delivery even after tools have produced evidence.
- A naturally ended durable task resumes only through a server-owned `durable_task_continuations` descriptor. A narrow standalone `continue` claims the row atomically and sends normal Runtime `input` on the existing Runtime thread. Checkpoint-backed clarification remains the only path allowed to send `command.resume`; the two protocols must never fall back to each other.
- Requeued continuation attempts retain the same `deliveryId` and a persisted chain of safe evidence run ids. Restoration unions and deduplicates only delivery-matching completed tool/file/output/Canvas evidence; lifecycle, failure, error, cross-delivery, and arbitrary client context are excluded.
- `clientRequestId` replay is resolved before continuation claim and Runtime I/O for both sync and streaming entry points. Run recording keeps its minimal repository dedupe contract, while generation replay reconstructs the stored public response and current continuation summary.
- `resume_context_json` stores only the runtime resume handles needed by the backend adapter, such as runtime thread, run, interrupt, and checkpoint ids. Answering a persisted Agent clarification uses the AgentBackend resume path when those handles exist. Missing handles are treated as recoverable continuation errors; the service must not silently fall back to re-asking through a fresh run.
- Agent Runtime clarification is stateful but not terminal by itself. `agent_backend_agent_clarification_requested` records a waiting state only when no later substantive progress exists; later evidence tools, file tools, or committed Canvas delivery events clear the waiting interpretation and allow the run to complete. Plan clarification, Plan approval, and Canvas write approval remain FacetWrite-owned product flows and do not use Runtime checkpoint resume.
- Runtime budget and internal-output safety markers are not completion blockers by themselves. `synthesis_gate` and ordinary model waiting/retry timeline entries must not be interpreted as user clarification. `internal_output_blocked` removes unsafe visible text and records a diagnostic event, but the run completion verdict still comes from final text, durable delivery evidence, explicit valid clarification, or real runtime failure.
...
- Long-task run budgets are explicit request controls. The workspace composer sends `runtimeBudgetProfile` (`low`, `medium`, or `high`, default `low`), the TypeScript generation service maps that profile to `facetwrite_recursion_limit`, `facetwrite_model_call_limit`, `facetwrite_evidence_tool_limit`, `facetwrite_body_draft_write_limit`, and `facetwrite_synthesis_reserve_steps`, and the AgentBackend adapter keeps those runtime context fields as product budget signals. Current defaults are low `8/2/18/80/16`, medium `12/3/24/110/22`, and high `16/4/32/140/28`. The adapter and Gateway send a larger top-level LangGraph `config.recursion_limit` hard guard (`max(160, advisory recursion * 2)`) as a runaway-loop fuse. Middleware emits `synthesis_gate`, adds a hidden final-synthesis reminder, narrows tools to finalization only, and gives budget-notice violations up to five finalization retries before marking the run as recoverable partial. File-delivery runs may still use `write_file` / `present_files`; checkpoint-only Canvas progress remains recoverable but is not terminal completion.
...
- Current validation target: `npm.cmd run acceptance:local-runtime` from `start-opencanvas-shell.vbs`, with Docker stopped, dynamic local Gateway ownership metadata, five real no-Mock UI generations, provider `agent-backend`, LangGraph clarification resume behavior, built-in ToolUse, Skills/MCP/Memory, and FacetWrite bridge ToolUse. Docker sidecar validation is separate and explicit.
...
- `durable_task_continuations` is the mutable owner of unfinished durable work; immutable `runs` remain execution history. Incomplete run recording and descriptor creation/requeue are one SQLite transaction, while claim and later transitions are guarded by state and claim token. A valid checkpoint-backed clarification may take ownership of `waiting`; an unresumable clarification leaves the durable task retryable.
...
- Project thumbnail cache path: `.facetwrite/project-thumbnails/<projectId>.png|webp` plus `.json` metadata. This cache is local-only, generated by the Electron shell from the visible Canvas surface, and is not synchronized to remote services. Deleting a Project removes its cached thumbnail files.
- Harness source updates may update tracked application files but must treat every storage path above as protected user/runtime data. Schema changes are applied by migrations only; source updates and future release packages must not replace `.facetwrite/data/facetwrite.db` or any user-data folder.
