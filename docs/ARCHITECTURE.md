...
- Long-task run budgets are explicit request controls. The workspace composer sends `runtimeBudgetProfile` (`low`, `medium`, or `high`, default `low`), the TypeScript generation service maps that profile to `facetwrite_recursion_limit`, `facetwrite_model_call_limit`, `facetwrite_evidence_tool_limit`, `facetwrite_body_draft_write_limit`, and `facetwrite_synthesis_reserve_steps`, and the AgentBackend adapter keeps those runtime context fields as advisory budget signals. Current advisory defaults are low `8/2/18/80/16`, medium `12/3/24/110/22`, and high `16/4/32/140/28`. The adapter and Gateway send a larger top-level LangGraph `config.recursion_limit` hard guard (`max(160, advisory recursion * 2)`) so final `write_file` / `present_files` delivery can finish after the advisory synthesis notice fires. Middleware emits `synthesis_gate` and a hidden final-synthesis reminder near the budget boundary without turning the budget into a tool kill switch.
