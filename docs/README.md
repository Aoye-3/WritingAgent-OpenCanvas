...
- `ARCHITECTURE.md`: frontend, backend, Agent runtime, AgentBackend sidecar, storage, and boundary rules.
- `CANVAS.md`: Canvas V2 frontend architecture, React Flow mapping, node geometry, resize behavior, and write-safety boundary.
- `AGENT.md`: AgentCard, Agent settings, AgentBackend runtime mapping, Tool catalog, and ToolUse safety.
- `API.md`: HTTP API contracts, runtime environment variables, and response shape.
- `AGENT_BACKEND_RUNTIME_RUNBOOK.md`: Docker Desktop/local and Linux Docker Compose runtime acceptance, dependencies, ports, env, smoke checks, and troubleshooting.
- `DATABASE.md`: SQLite location, tables, Canvas write semantics, and migration notes.
- `SECURITY.md`: local secret handling, tool permissions, AgentBackend auth, and runtime redaction rules.
...
- Agent, ToolUse, Skill, MCP, or AgentBackend runtime changes update `AGENT.md`.
...
AgentBackend is the AI execution/runtime plane. When enabled, FacetWrite calls the AgentBackend sidecar for Lead Agent, subagent, ToolUse, MCP, and intelligent orchestration behavior. The TypeScript provider runtime remains a fallback path while AgentBackend integration matures.
