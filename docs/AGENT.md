...
Long-task runs also carry a server-owned runtime budget profile: `low`, `medium`, or `high`. The project default is persisted per Project and defaults to `low`; the composer profile selector is a one-run override only. The profile maps to an advisory recursion budget, model-call budget, evidence-tool budget, evidence progress budget, body-draft checkpoint budget, and synthesis reserve steps. Budget thresholds are advisory synthesis pressure, not a tool kill switch: Agent Runtime middleware emits `synthesis_gate` telemetry and appends a hidden budget notice, but it must not clear the available tool list, force a second handler pass, or fail the run solely because the model continues with tools or internal protocol text. FacetWrite sends `facetwrite_recursion_limit` runtime context for middleware pressure signals and a larger top-level `config.recursion_limit` as the LangGraph hard guard; the Gateway expands the context value to that hard guard when a request would otherwise remain at its default.
...
Budget/synthesis signals are compatible with completed runs when final assistant text, a `file_document`, a final Body, or another durable delivery exists. Completion evaluation must not treat `synthesis_gate`, model-request waiting, retry, or other ordinary `run_timeline_decision` entries with `status:"waiting"` as pending user clarification.
