...
Approved Plan execution uses the same progressive Canvas delivery implementation as ordinary long tasks. Both streaming `generateAndRecordStream()` and non-streaming `generateAndRecord()` must wrap `plan_execution` payloads with the progressive delivery context before Agent Runtime runs. Execution finalization must commit the progressive final Body, file document, or other durable Canvas delivery first, then call Plan postcondition checks. A step may complete only after either a committed Plan artifact or a committed Canvas delivery event such as `canvas_delivery_body_final_committed` is present. If execution produces no committed artifact or Canvas node, the Plan remains recoverable and reports that the current step produced no delivery content instead of pretending the step completed.

Plan activity is persisted for audit and UI recovery, but raw runtime tool identifiers are not a user-facing progress language. `run_activities` and run trace UI should summarize tool lifecycle with safe categories such as source checking, workspace checking, Canvas update, or result organization. Low-value `tool_started` and `tool_completed` events remain available in stored tool/timeline payloads for debugging, but the default assistant Run Trace hides them unless they fail or require user action. Repeated `plan_ready`, `clarification_ready`, and low-value tool activity for the same Plan/step/status/summary should be deduplicated before it becomes chat-side activity.

Agent Runtime `ask_clarification` is a separate built-in interaction path, not a FacetWrite bridge tool and not Canvas delivery. Runtime pauses with LangGraph `interrupt(structured_payload)` and the Gateway resumes the same graph thread with `Command(resume=...)` after the user answers. The adapter emits `agent_backend_agent_clarification_requested`, and the generation service persists valid questions in `agent_clarifications` with `pending` or `answered` status. A run with a valid pending Agent clarification and no final deliverable is recorded as `clarification_required`/waiting, not completed. The right composer renders the choice card from persisted pending clarification state first; structured timeline payloads are only the recovery fallback. Answering a choice marks the stored row answered and, when `resumeContext.runtimeResume` is present, calls the AgentBackend resume path instead of starting a fresh ordinary run. The continuation must preserve the original instruction, `transientSkillRefs`, `disabledSkillRefs`, effective runtime budget profile, Canvas workflow metadata, and safe runtime resume handles. Ordinary Agent clarification keeps the single-question protocol but runs inside `ordinaryClarificationIntake` before execution. It may ask up to three rounds for the same original instruction; after the first answered round the request remains intake and only `ask_clarification` is exposed until two ordinary rounds have been answered, then `agent_intake_complete` can end intake. If the ordinary round limit is reached, execution proceeds without exposing ordinary `ask_clarification` again. If the scope guard still lacks required slots, the next request remains ask-only; once the guard is satisfied or the intake round limit is reached, the answered continuation is execution, not another generic intake checkpoint, and it must restore progressive Canvas delivery, evidence/file tools, and the product budget profile. If a Runtime-resume clarification lacks required runtime handles, the generation service returns a recoverable continuation error rather than silently re-asking. Structured Agent clarification must not create or update Canvas nodes; ordinary Markdown option lists, formatted ToolMessage content, and ordinary assistant prose are not upgraded into buttons. If a runtime cannot emit the tool call, it may output exactly one JSON object with `type:"agent_clarification_requested"`, `question`, and 2-3 structured `options`; any surrounding prose is invalid protocol output. If the Agent emits an invalid structured clarification only because it supplied too many options, the generation service performs one clarification-only repair pass that asks the same Agent to regenerate 2-3 options; only a second invalid result is exposed as a diagnostic.

Generic Agent intake is a hard boundary before execution. While `facetwrite_intake_phase:"intake"` is active, the request exposes only side-effect-free intake tools: `ask_clarification` plus the internal `agent_intake_complete` tool. `agent_intake_complete` emits `agent_backend_agent_intake_complete` and has no product side effects. The same generation orchestration then promotes the payload to `agentIntake.phase:"execution"` and starts a fresh execution run, where progressive Canvas delivery, `canvas_write`, `write_file`, `present_files`, evidence tools, runtime budgets, and file delivery policy are restored. The legacy `skill_scope_guard` is a narrower intake variant and intentionally remains ask-only on its first pass. Product Plan phases are separate from generic Agent intake: Plan intake/revision/preflight/execution keep their phase-specific tools and do not use `agent_intake_complete`.

Canvas context during generic Agent intake is metadata-only. Frontend clarification submission and backend runtime request construction both sanitize it with the shared intake sanitizer. Allowed fields are identifiers and routing metadata such as selected node id, node id/kind/title, workflow, and delivery id. Text-heavy fields such as `content`, `body`, `markdown`, `preview`, raw text, reference bodies, and file bodies must not reach the intake prompt or `facetwrite_context_values`. Execution may restore full delivery context after `agent_backend_agent_intake_complete`, or after an answered persisted clarification when the server scope guard has enough information to leave ask-only mode.

Clarification is blocking only while no later substantive Runtime progress exists. If the same run continues after `agent_backend_agent_clarification_requested` with evidence tools, file tools such as `write_file` or `present_files`, or committed Canvas delivery events, FacetWrite treats the clarification as historical context, normalizes the final run away from `clarification_required`, and finalizes the deliverable. A stale clarification event must not prevent progressive finalization, Plan postcondition checks, or file-document creation.
...
Long-task Canvas delivery is progressive and server-owned. During streaming runs in batch-delivery workflow, FacetWrite enables progressive Canvas delivery even when the user did not explicitly say "write to Canvas"; selected Skills, thinking mode, and any evidence-producing tool loop must not rely on the model to decide whether intermediate work should be stored. The server creates or updates stable `整体概述` / `Overview`, `正文草稿` / `Body draft`, and final `正文` / `Body` nodes, commits stable progress nodes only for completed evidence tool events that include sanitized linked `sources[]`, and updates the same draft node with recoverable checkpoints. The final Body node is reserved for the completed deliverable and must not be used as the checkpoint node. Explicit research-to-Canvas runs use `研究摘录 N` / `Research note N`; other long tasks use `进度摘录 N` / `Progress note N`.

Medium and long text deliverables should use file delivery instead of forcing the full Markdown into a Canvas content node. FacetWrite sends Agent Runtime a private Markdown delivery policy: after two or more `web_search` rounds, or when a complex Skill is expected to produce a long report, the runtime should write the complete Markdown to `/mnt/user-data/outputs/*.md` with `write_file` and then call `present_files`. FacetWrite then creates or updates one stable `file_document` Canvas node for the output file. Runtime-authored Markdown paths are preferred over server fallback files: if artifact archiving fails but the Thread outputs directory contains a readable Markdown file for the reported path, the real file remains the preview document. If Runtime does not create a readable file, final progressive delivery writes a fallback Markdown file under the current thread outputs directory and creates the same `file_document` entry. The node is a compact entry point with filename, virtual path, status, and preview action; it is not the full document and is excluded from Project context by default. Lightweight answers and short Canvas content still use ordinary `document` nodes.

`present_files` is a final delivery signal even when the final assistant chat text is empty. When file-tool events identify `/mnt/user-data/outputs/*.md`, backend finalization must create or update the `file_document` node before deciding whether the visible assistant text is empty.

Long-task runs also carry a server-owned runtime budget profile: `low`, `medium`, or `high`. The project default is persisted per Project and defaults to `low`; the composer profile selector is a one-run override only. The profile maps to an advisory recursion budget, model-call budget, evidence-tool budget, evidence progress budget, body-draft checkpoint budget, and synthesis reserve steps. Budget handling is layered: the profile is the product budget, middleware emits `synthesis_gate` telemetry and appends a hidden budget notice at the soft gate, tool exposure narrows to finalization tools at the synthesis gate, and model attempts to continue with exploration tool calls or internal tool protocol after that notice are blocked with a finalization message. FacetWrite sends `facetwrite_recursion_limit` runtime context for middleware pressure signals and a larger top-level `config.recursion_limit` as the LangGraph hard guard; the Gateway expands the context value to that hard guard when a request would otherwise remain at its default. The LangGraph recursion limit is a fuse for runaway loops, not the normal budget-stop path.

The long-task budget is evidence-first. Evidence-tool budget controls how many material-gathering completions may create research/progress nodes. Body draft writes are no longer a hard stop and must not by themselves trigger final synthesis; the stable `正文草稿` / `Body draft` node can keep receiving recoverable checkpoints while the run continues. Defaults are low `8 evidence / 2 body drafts / 18 model calls / 80 recursion / 16 reserve`, medium `12 / 3 / 24 / 110 / 22`, and high `16 / 4 / 32 / 140 / 28`. When the evidence budget enters synthesis territory, the server emits `canvas_delivery_synthesis_started`; later tool events may remain in the run trace, but evidence progress creation is bounded by the evidence budget.

Budget/synthesis signals are compatible with completed runs when final assistant text, a `file_document`, a final Body, a committed Canvas mutation, or a committed Artifact exists. Completion evaluation must not treat `synthesis_gate`, model-request waiting, retry, or other ordinary `run_timeline_decision` entries with `status:"waiting"` as pending user clarification. Canvas research/progress nodes and `Body draft` checkpoints are recoverable intermediate artifacts; they preserve work across retries and budget stops, but they are not terminal delivery and must not by themselves complete a run.

Progress nodes contain only allowlisted fields from completed tool events: `toolName`, `query`, `url`, `path`, `title`, `snippet`, `summary`, and `sourceCount` or sanitized `sources`. A progress/research excerpt node is created only when sanitized `sources[]` contains at least one HTTP(S) URL; top-level `url`, local `path`, `summary`, `snippet`, or command output alone is not enough. They cover `web_search`, `web_fetch`, `read_file`, and safe summaries from `grep`, `glob`, `ls`, and `bash` status when linked sources are present. They never include raw tool JSON, prompts, provider reasoning, request headers, environment variables, credentials, or hidden chain-of-thought. They are intentionally retained if the later Agent Runtime run fails, so long tasks do not lose already gathered linked work.
...
Agent settings are the user-controlled configuration surface for concrete Agent profiles. They define intent, prompts, Skills, tool refs, MCP refs, memory, and knowledge scope. They do not own model identity or provider credentials. Agent Runtime is the execution subsystem that consumes these settings through FacetWrite's adapter contract; the current implementation is the LangGraph-compatible AgentBackend Gateway.
...
Internal runtime output is redacted from visible assistant text and recorded as `internal_output_blocked` with safe diagnostic metadata. This event is an error marker, not a run terminator. If no user-visible text remains, the recorded assistant output may be empty; existing Canvas, file, Plan, or Artifact delivery events still determine whether the run completed. Absence of a visible deliverable is a partial or empty-output completion concern, not an automatic `agent_backend_runtime_failed`.
...
The visible run trace is not chain-of-thought. Tool lifecycle messages such as "using write_file" or "canvas returned" are low-level execution events, while user-facing progress narration should be a safe summary of intent, milestones, decisions, and next steps. Runtime may emit deterministic public narration at real execution boundaries through `agent_progress_reported` / frontend `progress_event` payloads with `visibility:"public"` and `source:"agent_public_update"`. Public updates may include only a `summary`, optional `next`, and bounded `evidence` chips; they must not include prompts, provider reasoning, replayed messages, raw tool JSON, tool-call arguments, environment details, credentials, or raw outputs. The frontend renders public updates through `progressSegments` inside the assistant run block and keeps raw logs separate.
...
Direct Canvas delivery also uses those sanitized sources. Source links are collected from final Canvas content, committed `canvas_write` content, and tool events, deduplicated by URL, and written to a dedicated `来源` / `Sources` reference node when available. Markdown links from the final document are preferred over broad search-result links so an authored reference list is not displaced by the first `web_search` page.

Streaming progress nodes are not final-output evidence by themselves. A run with only progressive `canvas_delivery_research_committed` or `canvas_delivery_body_checkpoint_committed` events and no final assistant text still fails as a runtime error; the preserved progress nodes and `正文草稿` / `Body draft` checkpoint are recoverable work product, not proof that the assistant completed the requested task. Checkpoint events are draft-node hints and require Thread state refresh for full Canvas content. On successful generic progressive delivery, the final assistant text is committed to the separate final `正文` / `Body` node and emits `canvas_delivery_body_final_committed`. Explicit direct Canvas delivery still uses the dedicated structured delivery planner and is not overwritten by the generic final-body path. On runtime failure, the server updates `整体概述` / `Overview`, commits a `运行失败` / `Run failed` summary node, and emits `canvas_delivery_failed_summary_committed`.

Current progressive delivery semantics: `canvas_delivery_body_checkpoint_committed` updates only the stable `正文草稿` / `Body draft` node and is bounded by `bodyDraftWriteLimit`. Checkpoint live payloads carry `nodeId`, `title`, `displayTitle`, `contentPreview`, and `contentHash` hints, not full node content. Full Canvas content is reconciled through Thread state refresh. Only `canvas_delivery_body_final_committed` means the final `正文` / `Body` node was written.

When a user enables a research-scope Skill for an under-scoped research/search request, the generation service applies a deterministic scope guard before AgentBackend runs evidence tools. The guarded Skill family currently includes `database-lookup`, `paper-lookup`, `literature-review`, `systematic-literature-review`, `deep-research`, `github-deep-research`, `citation-management`, `newsletter-generation`, and `consulting-analysis`. The guard still invokes AgentBackend with the loaded Skill context, but the pre-search phase exposes only `ask_clarification`; evidence tools such as `web_search`, `web_fetch`, and `knowledge_base` are withheld until the intake state is sufficient. The intake state is carried through `facetwrite_clarification_policy` and clarification `resumeContext` as `intakeRound`, `maxIntakeRounds`, `answeredSummary`, `answeredSlots`, and `missingSlots`. It may ask up to three focused rounds for slots such as topic/subdomain, time range, paper count/depth, citation format, output structure, and inclusion/exclusion criteria. Answering one clarification does not automatically bypass the guard: if required slots are still missing and the round limit has not been reached, the next request remains `skill_scope_guard` and may ask again. When enough slots are answered, or the maximum intake round is reached, the answered request is promoted to `agentIntake.phase:"execution"` and restores normal Skill tools, progressive Canvas delivery, file delivery, evidence tools, and runtime budget context. Normal generic intake uses `agent_intake_complete` for its handoff; `skill_scope_guard` uses the server slot assessment to choose between another ask-only pass and execution.

The emitted `agent_backend_agent_clarification_requested` payload includes a safe `resumeContext` and is persisted in `agent_clarifications` so the frontend can send the original task plus the selected answer, preserving transient Skills, disabled Skills, effective runtime budget, and Canvas workflow. The generation service marks the matching clarification answered, strips synthetic `Selected clarification:` suffixes from the preserved original instruction, and merges the selected answer into the intake summary. If AgentBackend later repeats a clarification whose inferred slot is already answered, for example asking again for citation format after `APA 7` was selected, the generation service converts that event into `agent_backend_duplicate_clarification_suppressed` instead of persisting another pending `agent_clarifications` row. This suppression is intentionally server-side; the frontend should avoid broad semantic duplicate matching because it can hide legitimate follow-up questions. File-ingestion, analysis, creation, deployment, and ordinary writing Skills may still ask their own input-specific clarifying questions, but they do not use this evidence-tool scope guard by default.

Provider compatibility note: the scope guard expects structured Runtime clarification events for actionable user choices, but process narration such as "I loaded the skill" is classified as thinking/process output rather than ordinary assistant body text. If a provider rejects forced `tool_choice="ask_clarification"` under reasoning or thinking mode, the guard must disable those provider controls before forcing the tool. If the provider succeeds but the UI reports `missing_question`, inspect streamed partial tool-call chunks and the ToolMessage artifact bridge before changing frontend rendering. If the provider returns `too_many_options`, the generation service should repair once by rerunning the Agent in clarification-only mode; do not silently truncate options in Node or the frontend. Frontend code must still render only structured timeline payloads and Canvas finalization must still reject prose clarification text.
...
When Agent Runtime is active, the FacetWrite bridge tools still call the same policy and executor path. This keeps disabled tools, disallowed Agent tools, and approval-gated writes consistent across Agent Runtime and compatibility-only local tool paths. The legacy TypeScript provider loop is not a maintained generation fallback.
