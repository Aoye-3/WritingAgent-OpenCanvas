...
The visible run trace is not chain-of-thought. Tool lifecycle messages such as "using write_file" or "canvas returned" are low-level execution events, while user-facing progress narration should be a safe summary of intent, milestones, decisions, and next steps. Future trace UI should aggregate repetitive tool events into readable execution summaries without exposing hidden reasoning, prompts, raw tool payloads, or provider-private thinking fields.
