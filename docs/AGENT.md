...
When a user enables a research-scope Skill for an under-scoped research/search request, the generation service applies a deterministic scope guard before AgentBackend runs evidence tools. The guarded Skill family currently includes `database-lookup`, `paper-lookup`, `literature-review`, `systematic-literature-review`, `deep-research`, `github-deep-research`, `citation-management`, `newsletter-generation`, and `consulting-analysis`. The guard still invokes AgentBackend with the loaded Skill context, but the first run is a clarification-only phase: only `ask_clarification` is exposed, thinking/reasoning controls are disabled for forced-tool provider compatibility, and Canvas/progressive/file/evidence delivery context is removed from the Runtime request. Evidence tools such as `web_search`, `web_fetch`, and `knowledge_base` are not exposed until the user answers, and the first guarded run must not create Canvas nodes. The clarification question and options come from Agent Runtime under the Skill prompt, not from a fixed server template. The emitted `agent_backend_agent_clarification_requested` payload includes a safe `resumeContext` and is persisted in `agent_clarifications` so the frontend can send the original task plus the selected answer, preserving transient Skills, disabled Skills, effective runtime budget, and Canvas workflow. The next run includes the selected answer in `requestContext.agentClarification`, marks the matching clarification answered, bypasses the guard, restores normal provider thinking settings, and proceeds through AgentBackend with progressive Canvas delivery still eligible for long Skill tasks. File-ingestion, analysis, creation, deployment, and ordinary writing Skills may still ask their own input-specific clarifying questions, but they do not use this evidence-tool scope guard by default.

Provider compatibility note: the scope guard requires a structured Runtime clarification event and must fail closed when none is produced. If a provider rejects forced `tool_choice="ask_clarification"` under reasoning or thinking mode, the phase-one guard must disable those provider controls before forcing the tool. If the provider succeeds but the UI reports `missing_question`, inspect streamed partial tool-call chunks and the ToolMessage artifact bridge before changing frontend rendering. Frontend code must still render only structured timeline payloads and Canvas finalization must still reject prose clarification text.
