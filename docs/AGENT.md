...
Long-task runs also carry a server-owned runtime budget profile: `low`, `medium`, or `high`. The project default is persisted per Project and defaults to `low`; the composer profile selector is a one-run override only. The profile maps to LangGraph `recursion_limit`, model-call budget, evidence-tool budget, evidence progress budget, body-draft checkpoint budget, and synthesis reserve steps. Budget thresholds are advisory synthesis pressure, not a tool kill switch: Agent Runtime middleware emits `synthesis_gate` telemetry and appends a hidden budget notice, but it must not clear the available tool list, force a second handler pass, or fail the run solely because the model continues with tools or internal protocol text. FacetWrite sends both `facetwrite_recursion_limit` runtime context and top-level `config.recursion_limit`; the Gateway mirrors the context value to the top level when a request would otherwise remain at LangGraph's default 100.
