...
When a user enables `literature-review` or `database-lookup` for an under-scoped literature/database search request, the generation service performs a clarification preflight before AgentBackend runs. Without an answered `requestContext.agentClarification`, it emits `agent_backend_agent_clarification_requested` and does not call evidence tools such as `web_search`. The next run includes the selected answer in `requestContext.agentClarification`, bypasses the preflight, and proceeds through AgentBackend.
