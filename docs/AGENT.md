...
Long-task runs also carry a server-owned runtime budget profile: `low`, `medium`, or `high`, defaulting to `medium`. The profile maps to LangGraph `recursion_limit`, model-call budget, evidence-tool budget, and synthesis reserve steps. Agent Runtime middleware must stop exposing evidence tools and inject a final-synthesis instruction before the graph reaches the recursion clamp; increasing `recursion_limit` alone is not the fix.
