...
`canvas_write` directly commits low-risk create and append operations with stable IDs. New low-risk create nodes are placed by FacetWrite near the selected/target Canvas node, or near the current node group center when no anchor exists, while avoiding persisted node-rectangle overlap. Replace, range replacement, delete, and other destructive operations remain approval-gated. During progressive long-task delivery, `canvas_write` keeps the `short_progress_nodes` scope even when a server-recognized `canvasAction` requires the tool. Tool-call arguments are authoritative: when the Agent explicitly sends `operation:"create"` or `operation:"append"`, stale server `canvasAction.operation` context must not upgrade the write into replace/delete. `canvasAction.operation` is only a fallback when the tool call omitted `operation`. A fallback or explicit replace/delete with no valid target is skipped with `reason:"missing_target_node"` instead of creating a storage-level write request error.

The local sandbox tool surface also exposes file tools used by long-form deliverables: `read_file`, `list_directory`, `write_file`, and `present_files`. The model-visible `write_file` contract requires `description`, `path`, and `content`; `append` is optional. Runtime still accepts the legacy `file_path` alias for compatibility, but prompts should instruct Agents to emit `path` and place the full Markdown body in `content`. `present_files` accepts `filepaths`, with `file_paths` kept as an alias.
...
- Long runs that call `write_file` or `present_files` for `/mnt/user-data/outputs/*.md` must produce a `file_document` Canvas node. `write_file` calls must include `description`, `path`, and `content`; missing `path` or `content` is a tool-contract failure, not a filesystem or Canvas failure. The sanitized tool failure payload should preserve a safe `summary` such as `path is required` or `content is required` so the right drawer and logs can explain why the file was not written. If Runtime does not call file tools but the run is a medium/long progressive Canvas delivery, Node finalization writes a fallback Markdown file under `.facetwrite/threads/<threadId>/user-data/outputs/` and creates the same document node. If artifact archiving fails but that thread output directory already contains the reported Markdown file, the real Runtime file should still become the preview document and fallback must not shadow it. If the timeline shows completed file tools but no document node, inspect the safe tool args in the AgentBackend adapter, the local archived output path, the `canvas_delivery_file_document_committed` event, and `metadata.fileDocument.path`. The node should contain only a short file summary; the full Markdown is loaded through `/api/threads/:threadId/canvas/document-preview`.
- If a final answer claims that all Canvas and file writes failed, split the diagnosis by channel before trusting the summary. Direct Agent `canvas_write` can fail while server-owned progressive delivery still committed `canvas_delivery_*` nodes. File delivery failure should be tied to the concrete `write_file` / `present_files` tool event and its safe error summary, especially `path is required` or `content is required`.
...
- If the timeline contains `synthesis_gate` with `finalization_retry_exhausted:true`, the budget gate worked but the model kept requesting unavailable exploration tools or emitting internal tool protocol through five finalization attempts. Treat the run as recoverable partial work, not as a completed answer and not as proof that the LangGraph `recursion_limit` is too low. The right drawer should offer the budget-continuation draft; the next request should preserve the existing clarification answers, Skills, Canvas workflow, and budget profile while asking the Agent to continue finalization from gathered evidence.
...
`finishReason:"clarification_required"` means the run is waiting; it does not by itself prove the next answer can use LangGraph resume. The server, not the frontend request, is authoritative for `runtimeResume`. A submitted answer is persisted first, then the stored row moves through `awaiting_answer -> queued -> resuming -> succeeded|failed`. Missing metadata fails with `clarification_resume_metadata_missing`; it must never fall back to a normal Runtime `input` run.

The persisted row also records `resume_attempts`, `last_resume_error`, and `resumed_runtime_run_id`. A failed resume retains the answer and checkpoint so the UI can offer **Retry resume** without reopening the choices. Only a failure explicitly known to happen before Runtime accepts the stream is retried automatically, once, against the same checkpoint. Ambiguous or post-acceptance failures remain `failed` for an explicit retry.

Frontend recovery UI follows the stream boundary. Persisted `queued`/`resuming` rows may display a resume notice after refresh when no local generation stream is active. While `isSending` is true, hide that transitional notice and use the normal Runtime progress plus composer stop/queue controls; otherwise a long resumed research run looks permanently stuck on "resuming". Always keep a `failed` resume card visible so **Retry resume** remains available.

When debugging "answering a clarification starts a new run", inspect the persisted clarification before the frontend queue. A correct resume continuation posts to Runtime with `command: { resume: ... }`, `metadata.resumeOfRunId`, and the interrupt/checkpoint ids. A normal run with `command:null` is a protocol violation for a persisted clarification answer. If the stream contains both `source:"ask_clarification"` and `source:"runtime_interrupt"` for the same question/options, adapter, generation-service, and repository dedupe must keep or merge the interrupt version rather than letting the earlier tool-call event win.

The accepted design, rejected fallbacks, and retry boundary are recorded in [`ADR-2026-07-14-durable-agent-clarification-checkpoint-resume.md`](decisions/ADR-2026-07-14-durable-agent-clarification-checkpoint-resume.md).
...
The composer may show a budget-continuation prompt after a recursion-limit failure or an exhausted budget-finalization retry. It only drafts a user-visible continue instruction so the user can edit and resubmit; it does not convert the failed or partial run into success, and the next request must still preserve the previous clarification answer, Skills, Canvas workflow, and budget semantics.
