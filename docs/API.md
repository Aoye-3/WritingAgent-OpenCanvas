...
- `POST /api/generate/stream`
  ...
  - Before AgentBackend is invoked, requests that enable `literature-review` or `database-lookup` and look like under-scoped literature/database search tasks run a server preflight. If the request lacks an answered `requestContext.agentClarification`, the backend emits `agent_backend_agent_clarification_requested`, records a waiting run timeline event, and does not start evidence tools such as `web_search`.
