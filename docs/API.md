...
- `POST /api/generate/stream`
  ...
  - Agent Runtime `ask_clarification` tool calls are adapted into `agent_backend_agent_clarification_requested` events and mirrored as waiting run timeline events. The right composer derives its button choice card from that structured timeline payload. These clarifications are conversation input state, not Canvas delivery content; ordinary Markdown or prose option lists in assistant text are not parsed into buttons.
