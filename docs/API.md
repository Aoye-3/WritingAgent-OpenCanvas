...
- `POST /api/generate`
  ...
  - `clientRequestId`, when present, is idempotent within the Thread. The service checks persisted runs before claiming a durable continuation or making Runtime I/O. A replay returns the stored text, completion verdict, safe events, Runtime ids, usage, and the current public continuation summary without executing the Agent again.
  - A standalone continuation instruction such as `continue`, `go on`, or `接着做` claims only a server-persisted `ready` or `failed` durable continuation. Runtime receives a server-reconstructed normal `input` on the same Runtime thread, with the original Skills, budget, Plan position, `deliveryId`, Canvas scope, and safe evidence. Client `contextValues` cannot create or override this descriptor. If no active descriptor exists, the text remains an ordinary message.
  - `GenerateResponse.completion` is the authoritative run verdict. `completed` is terminal; `continue`, `partial`, `waiting`, `failed`, and `finalizing` are nonterminal. `GenerateResponse.durableContinuation`, when present, is limited to `{ state, canContinue, attempts, lastError? }`; `canContinue` is true only for `ready` and `failed`.
  - Uses Agent Runtime as the only real generation path. Runtime/model failures return stable error codes and do not record an assistant message, output version, or Mock result. Redacted internal-output leaks are different from runtime failures: they emit `internal_output_blocked`, remove the visible text, and still record the AgentBackend run when the runtime itself completed.
...
- `POST /api/generate/stream`
  - Request body accepts `runtimeBudgetProfile?: "low" | "medium" | "high"`. When omitted, generation uses the current Project runtime settings; when present, the value is a one-run composer override. The value controls the advisory recursion budget, model-call budget, evidence-tool budget, body-draft write budget, and synthesis reserve for the single request; it does not change model thinking settings. Progressive runs send `facetwrite_recursion_limit` as middleware budget context and a larger top-level LangGraph `config.recursion_limit` hard guard; the Runtime Gateway expands the context value to that hard guard when the top-level value would otherwise remain the default.
  - SSE endpoint.
  ...
  - Agent Runtime `ask_clarification` pauses the underlying LangGraph run with a native checkpoint interrupt. Runtime interrupt stream events are adapted into `agent_backend_agent_clarification_requested` and mirrored as waiting run timeline events. The event `resumeContext` includes product continuation fields plus `runtimeResume` handles for the backend adapter, such as runtime thread id, run id, interrupt id, and optional checkpoint id. Clients must persist and return the structured answer through the existing Agent clarification submission path; they must not expose raw checkpoint state, prompts, reasoning, or tool arguments.
  - Answering a Runtime-backed Agent clarification resumes the same AgentBackend graph thread. The generation service sends `command.resume` to the Runtime Gateway through `resumeAgentBackendRun`; it does not convert the answer into a new ordinary chat instruction. If a pending Agent clarification requires Runtime resume but lacks usable `runtimeResume` metadata, the API returns a recoverable continuation error instead of silently starting over.
  - Agent clarifications are conversation input state, not Canvas delivery content; ordinary Markdown or prose option lists in assistant text, formatted ToolMessage content, and partial chunks are not parsed into buttons. For Skill scope clarification, the event payload includes `resumeContext` with the original instruction, transient Skills, disabled Skills, effective runtime budget profile, and Canvas workflow; clients must preserve those values when sending the selected answer. If Runtime emits only a partial product `resumeContext`, the generation service fills missing non-runtime fields before forwarding the event. Invalid Agent clarification payloads are diagnostics only and may expose safe fields such as `reason`, `hasQuestion`, `optionCount`, and `optionShape`; clients must not render empty choice cards from them.
  ...
  - `canvas_delivery_research_committed` means a progressive research/progress node was committed during a server-owned long-task Canvas delivery run. `canvas_delivery_synthesis_started` means a runtime budget threshold was reached and the run should prefer final synthesis from available evidence. It is not a failure signal, does not disable tools by itself, and may still result in a completed run when final assistant content or a valid final structured Plan/Artifact/Canvas lifecycle outcome exists. `canvas_delivery_failed_summary_committed` means the server preserved a visible failure summary node before returning a real runtime error.
  ...
  - If the stream fails after visible text arrives, clients must refresh `GET /api/threads/:threadId/state` before applying a local error fallback. Persisted `ready`, `claimed`, or `failed` continuation state is authoritative even when the stored text exactly matches the live text.
  - `durable_task_incomplete` from Runtime is adapted to `finishReason:"agent_backend_incomplete"`. The server records `run_incomplete`, skips terminal Canvas/Plan side effects, and persists or requeues the continuation in the same transaction as the run.
...
- `POST /api/internal/agent-runtime/tool-call`
  ...
  - During progressive delivery, `canvas_write` keeps `facetwrite_canvas_write_scope:"short_progress_nodes"` even when a structured `canvasAction` requires the tool. If a forced `replace` or `delete` has no valid target node, the response is `ok:false` with `payload.eventType:"canvas_mutation_failed"` and `payload.reason:"missing_target_node"`; clients should treat it as recoverable diagnostics, not as failure of `canvas_delivery` finalization.
...
- `GET /api/threads/:threadId/state`
  - Returns thread, sanitized messages, sanitized output versions, tool events, Canvas nodes, Canvas edges, Canvas Workflow state, Canvas Workflow suggestions, pending Canvas write requests, ordinary Canvas write suggestions, Plans, Plan activities, the latest `runCompletion`, and optional `durableContinuation`.
  - `durableContinuation` is derived from SQLite and contains only `{ state, canContinue, attempts, lastError? }`. Descriptor JSON, claim token/time, source run ids, Skills, budget, `deliveryId`, checkpoint credentials, and internal policies are never returned.
  - `ready` and `failed` allow an explicit continuation; `claimed` reports work already being restored; `completed` and `superseded` are non-actionable. Refresh and stream-error recovery must use this server state rather than React optimistic state.
...
Explicit Canvas instructions are converted server-side into a structured `canvasAction`. Internal Runtime Bridge calls always resolve `projectId` from `threadId`; a mismatched Runtime-supplied project is rejected. `canvas_write` returns either `{ status:"committed", nodeId, projectId, operation }` for create/append, `{ status:"pending", requestId, projectId, operation }` for targeted destructive writes awaiting approval, or `ok:false` / `reason:"missing_target_node"` for untargeted forced update/delete attempts.
