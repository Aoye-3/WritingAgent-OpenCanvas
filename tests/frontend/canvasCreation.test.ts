...
test("maps node tools to their persisted kinds and default sizes", () => {
  assert.deepEqual(getCanvasCreationSize("document"), { width: 520, height: 260 });
  assert.deepEqual(getCanvasCreationSize("note"), { width: 300, height: 190 });
  assert.deepEqual(getCanvasCreationSize("reference"), { width: 300, height: 190 });
  assert.deepEqual(getCanvasCreationSize("role"), { width: 280, height: 190 });
