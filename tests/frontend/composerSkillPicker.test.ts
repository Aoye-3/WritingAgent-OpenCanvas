...
test("workspace skill picker uses folder catalog and per-message overrides", () => {
  ...
  const composer = readFileSync("src/features/workspace/components/AIComposer.tsx", "utf8");
  ...
  const dialog = readFileSync("src/features/workspace/components/SkillPickerDialog.tsx", "utf8");
  ...
  assert.match(composer, /SkillPickerDialog/);
  assert.match(composer, /aria-haspopup="dialog"/);
  assert.match(toolbar, /toolbar-skill-picker/);
  assert.match(toolbar, /SkillPickerDialog/);
  assert.match(toolbar, /aria-haspopup="dialog"/);
  assert.match(toolbar, /onCreateSkillFolder/);
  assert.doesNotMatch(source, /SkillFolderPicker/);
  assert.match(source, /transientSkillRefs/);
  assert.match(source, /disabledSkillRefs/);
  assert.match(dialog, /createPortal/);
  assert.match(dialog, /SkillFolderPicker/);
  assert.match(dialog, /aria-modal="true"/);
  assert.match(dialog, /event\.key === "Escape"/);
  assert.match(styles, /\.skill-picker-dialog-backdrop/);
  assert.match(styles, /z-index: var\(--z-modal\)/);
  assert.match(styles, /\.skill-picker-dialog-body/);
  assert.doesNotMatch(styles, /\.composer-skill-menu/);
  assert.doesNotMatch(styles, /\.board-skill-menu/);
  assert.doesNotMatch(composer, /composer-skill-menu/);
  assert.doesNotMatch(toolbar, /board-skill-menu/);
